"""Mock safe MCP server for local InvokeCordon demos. No real side effects."""
...
app = FastAPI(title="InvokeCordon safe MCP mock")
...
TOOLS: list[dict[str, Any]] = [
    ...
    {
        "name": "summarize_public_text",
        "description": "Summarize a short public-domain text snippet provided by the caller.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "text": {"type": "string"},
...
def handle_initialize(req_id: Any) -> dict[str, Any]:
    return jsonrpc_result(
        req_id,
        {
            ...
            "capabilities": {"tools": {}},
            "serverInfo": {"name": "invokecordon-safe-mock", "version": "0.1.0"},
        },
    )
...
def mock_tool_text(name: str, arguments: dict[str, Any]) -> str:
    ...
    if name == "summarize_public_text":
        text = str(arguments.get("text", ""))
        if len(text) > 80:
            return f"mock summary: {text[:80]}..."
