# InvokeCordon threat model

InvokeCordon is a local scanner and (planned) runtime policy gateway for MCP servers. It inspects advertised tools and, later, will intercept tool calls. It is unofficial and not affiliated with the MCP project.

## What InvokeCordon protects against
...
- Network attackers who can talk to the MCP server without going through InvokeCordon.
- Supply-chain attacks in dependencies of the MCP server or of InvokeCordon.
- Authorization, identity, or multi-tenant isolation (InvokeCordon does not implement auth).
...
3. **SSRF to metadata.** `fetch_url` is pointed at a cloud instance-metadata address. Policy should deny the tool or block metadata URLs. InvokeCordon cannot stop a server that ignores the gateway.
...
- Deny shell, filesystem, and arbitrary HTTP tools unless explicitly required and sandboxed outside InvokeCordon.
